...
export const Card = ({ title, getUrl, rid }) => {
  ...
  const limitedList = story.slice(0, 15)
        ...
        {limitedList.map(paper => (
          <div className={`${rid ? 'card-left' : 'card-right'}`} key={paper.title}>
            ...
            <Link to={`/cardDetails/${paper.title ? paper.section : <>Having some trouble please try again </>}`}
              state={{ paper }}>
              { paper.multimedia[0] ? <img src={paper?.multimedia[0].url} /> : <img src='https://static01.nyt.com/images/2023/01/11/multimedia/11dc-bidendocs-vwlq/11dc-bidendocs-vwlq-threeByTwoMediumAt2X.jpg?format=pjpg&quality=75&auto=webp&disable=upscale' />}
            </Link>
          </div>
        ))}
      </div>
